...
/// UI element which uses a libadwaita Dialog to present the user
/// with information about the application, and options to modify
...
impl SettingsMenu {
    ...
    pub fn new() -> Self {
        ...
        // Registering a callback to change the theme colour when the dropdown changes
        theme_dropdown.connect_selected_notify(|dropdown| {
            let index = dropdown.selected();

            let colour_scheme = match index {
                1 => ColorScheme::ForceLight,
                2 => ColorScheme::ForceDark,
                _ => ColorScheme::Default, // Including 0
            };
    ...
    /// Displays the settings menu on the provided window.
